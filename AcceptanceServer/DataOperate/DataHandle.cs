...
namespace AcceptanceServer.DataOperate
{
    public static class DataHandle
    {
        ...
        {
            string sql = string.Format(@"insert into [Message](SystemStatus,TextContent,EnumMessageSendDirectionID,EnumMessageTypeID,FromAccountID,FromUserID,ToAccountID,ToUserID,IsReceive,SendTime,ReceiveTime,fileUrl) 
                                         values({0},'{1}',{2},{3},{4},{5},{6},{7},'false',getdate(),getdate(),'{8}') select @@IDENTITY as TID",
        ...
        {
            string sql = string.Format("insert into PendingMessages(SystemStatus,FromAccountID,FromUserID,ToAccountID,ToUserID,SendTime,Content,MessageID,EnumMessageTypeID,MSD,fileUrl) values(0,{0},{1},{2},{3},getdate(),'{4}',{5},{6},'{7}','{8}')",
