...
namespace Business.MerchantBusiness
{
    ...
    public class M_DomesticModel : BaseModel<M_Domestic>, IM_DomesticModel
    {
        ...
        /// <returns></returns>
        public M_Domestic GetInfoByID(int MID, int DID)
        {
            var item = List().Where(a => a.MerchantID == MID && a.ID == DID).FirstOrDefault();
            return item;
        ...
        /// <returns></returns>
        public IQueryable<M_Domestic> GetListByStatus(int? EnumDataStatus, string CreatDate, string MName)
        {
        ...
        /// <returns></returns>
        [Transaction]
        public Result EditInfo(M_Domestic m_domestic, int[] communityIDs, int x1, int y1, int width, int height, int Twidth, int Theight)
        {
            Result result = new Result();
            var Ym_domestic = base.Get(m_domestic.ID);

            if (Ym_domestic.MainImage != m_domestic.MainImage)
            {
                if (width > 0)
                {
                    CommonModel com = new CommonModel();
            ...
            string sql = "DELETE dbo.M_CommunityMapping WHERE M_DomesticID=" + m_domestic.ID;
            base.SqlExecute(sql);

            result = base.Edit(m_domestic);


            StringBuilder sql_add = new StringBuilder();
            sql_add.Append("INSERT INTO dbo.M_CommunityMapping( SystemStatus , AccountMainID , M_DomesticID )");
            ...
            for (int i = 0; i < communityIDs.Length; i++)
            {
                ...
                {
                    sql_add.AppendFormat("SELECT 0,{0},{1} ", communityIDs[i], m_domestic.ID);
                }
                else
                {
                    sql_add.AppendFormat("SELECT 0,{0},{1} UNION ALL ", communityIDs[i], m_domestic.ID);
                }
            }
        ...
        public Result DeleteInfo(int DID, int MID)
        {
            ...
            string sql2 = "DELETE dbo.M_CommunityMapping WHERE M_DomesticID=" + DID;
            int cnt = base.SqlExecute(sql2);

            string sql = string.Format("Delete M_Domestic where id={0} and MerchantID={1}", DID, MID);
        ...
        {
            Result result = new Result();
            string sql = string.Format("update M_Domestic set EnumDataStatus= {0} where ID = {1}", EnumDataStatus, DID);
            if (EnumDataStatus == (int)Poco.Enum.EnumDataStatus.Disabled)
            {
                sql = string.Format("update M_Domestic set EnumDataStatus= {0}, IsPublish='False' where ID = {1}", EnumDataStatus, DID);
            }
        ...
        public Result UpdatePush(int DID, bool push)
        {
            ...
            if (push)
            {
                sql = string.Format("update M_Domestic set IsPublish= '{0}',publishdate='{1}' where ID = {2}", push, DateTime.Now.ToString(), DID);
            }
            else
            {
                sql = string.Format("update M_Domestic set IsPublish= '{0}',publishdate=null  where ID = {1}", push, DID);
            }
