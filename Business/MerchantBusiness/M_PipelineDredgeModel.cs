...
namespace Business.MerchantBusiness
{
    public class M_PipelineDredgeModel : BaseModel<M_PipelineDredge>, IM_PipelineDredgeModel
    {
        ...
        public Result UpdatePush(int UID, bool push)
        {
            ...
            if (push)
            {
                sql = string.Format("update M_PipelineDredge set IsPublish= '{0}',publishdate='{1}' where ID = {2}", push,DateTime.Now.ToString(), UID);
            }
            else {
                sql = string.Format("update M_PipelineDredge set IsPublish= '{0}',publishdate=null  where ID = {1}", push, UID);
            }
