...
namespace Business
{
    ...
    /// 每日食谱
    /// </summary>
    public class RecipesModel : BaseModel<Recipes>, IRecipesModel
    {
        /// <summary>
        /// 获取列表
        /// </summary>
        ...
        /// <returns></returns>
        public IQueryable<Recipes> GetList()
        {
        ...
        /// <returns></returns>
        public Recipes GetInfo(int RID)
        {
            return List().Where(a => a.ID == RID).AsNoTracking().FirstOrDefault();
        }
        ...
        {
            string sql = "update Recipes set BrowseCnt = (BrowseCnt+1) where id = " + id;
            base.SqlExecute(sql);
        }
        ...
        /// <returns></returns>
        [Transaction]
        public Result AddRecipes(Recipes recipes, int w, int h, int x1, int y1, int tw, int th)
        {
            ...
            try
            {
                ...
                var LastName = com.CreateRandom("", 5) + recipes.MainImagPath.GetFileSuffix();
                ...
                var lsImgPath = recipes.MainImagPath;
                ...
                recipes.MainImagPath = path + imageName;
                recipes.AppShowImagePath = path + imageshowName;
                recipes.MinImagePath = path + imageminiName;
        ...
        /// <returns></returns>
        [Transaction]
        public Result EditRecipes(Recipes recipes, int w, int h, int x1, int y1, int tw, int th)
        {
            var appadvertorials = this.GetInfo(recipes.ID);

            if (appadvertorials.MainImagPath != recipes.MainImagPath)
            {
                CommonModel com = new CommonModel();
                var LastName = com.CreateRandom("", 5) + recipes.MainImagPath.GetFileSuffix();
            ...
            Result result = base.Edit(recipes);
        ...
        [Transaction]
        public Result DelRecipes(int ID)
        {
        ...
        {
            string sql = string.Format("update Recipes set IsRelease='{0}' where ID={1}", Release,RID);
