...
namespace Web.Controllers
{
    public class LoginController : BaseController, System.Web.SessionState.IRequiresSessionState
    {
        ...
        [HttpPost]
        public ActionResult UserLogin(string phone_email, string password, int role)
        {
            switch (role)
            {
                case 1://物业
                ...
                case 2://商家
                    ...
                    var result3 = merchantModel.Login(phone_email, password);
                    if (result3.HasError)
                    {
                        return JavaScript("LandWaitFor('login','WaitImg',2);" + AlertJS_NoTag(new Dialog(result3.Error)));
                    }
                    var url3 = Url.RouteUrl("Merchant_default", new { action = "Index", controller = "MerchantHome" });
                    return JavaScript("window.location.href='" + url3 + "'");
                case 3://平台
                    ISystemUserModel systemUserModel = Factory.Get<ISystemUserModel>(SystemConst.IOC_Model.SystemUserModel);
                    var result2 = systemUserModel.Login(phone_email, password);
                    if (result2.HasError)
                    {
                        return JavaScript("LandWaitFor('login','WaitImg',2);" + AlertJS_NoTag(new Dialog(result2.Error)));
                    }
                    var url2 = Url.RouteUrl("System_default", new { action = "Index", controller = "SystemUserHome" });
                    return JavaScript("window.location.href='" + url2 + "'");
