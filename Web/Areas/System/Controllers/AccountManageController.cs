    ...
    public class AccountManageController : ManageSystemUserController
    {
        ...
        public ActionResult AddAccount(Account_AccountMain account_accountMain, string roleIDs, HttpPostedFileBase HeadImagePathFile, int w, int h, int x1, int y1, int tw, int th)
        {
            ...
            if (!string.IsNullOrEmpty(account_accountMain.Account.Email))
            {
                ...
                emailInfo.Body = string.Format("亲爱的用户:<br/><br/>您好！<br/><br/>您的{2}账号已创建成功,<a href='http://{0}.{1}' target='_blank'>请点击此处</a>&nbsp;登录。", Accountmain.HostName, SystemConst.WebUrl, SystemConst.PlatformName) +
        ...
        public ActionResult ResetPwd(int id, string mail)
        {
            ...
            if (!string.IsNullOrEmpty(mail))
            {
                ...
                emailInfo.IsHtml = true;
                emailInfo.UseSSL = false;
                emailInfo.Body = string.Format("亲爱的用户:<br/><br/>您好！<br/><br/>您的{1}账号密码重置成功,<a href='http://{0}.{2}' target='_blank'>请点击此处</a>&nbsp;登录。", account.Account_AccountMains.FirstOrDefault().AccountMain.HostName, SystemConst.PlatformName, SystemConst.WebUrl) +
